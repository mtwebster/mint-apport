...
and installing updates. Please free some space.' % (free_mb)
    ...
    if report.get('ProblemType') == 'Package' and 'ErrorMessage' in report and 'Package' in report:
        ...
        if 'trying to overwrite' in dupe_sig:
            conflict_pkg = re.search('in package (.*) ', dupe_sig)
        ...
        if report['Package'] not in ['grub', 'grub2']:
            ...
            for grub_error in grub_errors:
                if attachment in report and re.search(grub_error, log_file, re.MULTILINE):
                    ...
                    if grub_package is None or grub_package == 'grub' and not 'grub-probe' in log_file:
...
def add_release_info(report):
    ...
    if os.path.exists('/var/log/system-image/client.log'):
        report['SystemImageInfo'] = apport.hookutils.command_output(
