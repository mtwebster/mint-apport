...
def add_info(report):
    try:
        if apport.packaging.get_version('automatix') or apport.packaging.get_version('automatix2') or apport.packaging.get_version('ultamatix'):
...
will not be sent to the %s developers.' % report.get('DistroRelease', 'distribution').split()[0]
    except ValueError:
        return
